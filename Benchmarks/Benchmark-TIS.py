...
print('**Welcome to the Benchmark version of the Task Implementation Selector (TIS)** \n')
nTask = int(input('Enter the number of tasks: '))
nNodes = int(input('Enter the number of devices: '))
nConfigurations = int(input('Enter the number of configurations: '))

print(type(nConfigurations))
print('Runing the TIS for a problem with ' + str(nTask) + ' tasks, ' + str(nNodes) + ' nodes and ' + str(nConfigurations) + ' application configurations.')
...
for c in range (nConfigurations):
...
for c in range (nConfigurations):
	...
	timeAfter = time.time()
	if str(sat) == 'sat':
		energy = m[computationCost].numerator_as_long()/m[computationCost].denominator_as_long() + m[communicationCost].numerator_as_long()/m[communicationCost].denominator_as_long() 
		solutions.append([c,energy])

print('Execution time: ' + str(timeAfter-timeBefore))
